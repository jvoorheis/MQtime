*Version 0.4 of mqtime (updated 9/9/14)
...
program mqtime
	version 11
	syntax [in] [if], [start_x(string) start_y(string) end_x(string) end_y(string) start_add(string) end_add(string) api_key(string) km mode(string) time(string)]
	*di "`if'"
	qui {
			...
			*di "`mp_api_request'"
			insheetjson temp_time temp_distance errorcode temp_fuel using "`mp_api_request'", columns("route:formattedTime" "route:distance" "info:statuscode" "route:fuelUsed") flatten replace
				...
				*local time1 = temp_time in 1
				...
				noisily: di "Processed " `i' " of " `cnt' " using the commercial Mapquest API. (`mapquestcount' total requests this run)"
			}
